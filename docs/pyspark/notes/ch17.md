---
title: Deploying Spark
description: Compares on-premise vs cloud deployment and Spark's cluster managers - Standalone, YARN, and Mesos - and when to choose each.
tags:
  - concept
---
...
Spark supports the following cluster managers: Standalone, Hadoop YARN, Kubernetes, and Mesos (removed in Spark 3.2).
...
- Mesos deprecated fine-grained mode and exclusively supports coarse-grained mode, meaning that each Spark executor runs as a single Mesos task. NOTE: Apache Mesos support was removed in Spark 3.2. Use YARN or Kubernetes instead.
