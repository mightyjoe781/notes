...
However **this greedy doesn’t always work** for all sets of coin denominations e.g., {1, 3, 4} cents. To make 6 cents with this set, greedy picks 4+1+1 (3 coins) instead of the optimal 3+3 (2 coins).
...
````python
# dragon[] and knight[] sorted in non-decreasing order
gold = d = k = 0
while d < n and k < m:
    while k < m and dragon[d] > knight[k]:
        k += 1  # find knight tall enough
    if k == m:
        break  # no knight can slay this head
    gold += knight[k]
    d += 1; k += 1  # next dragon head and knight

if d == n:
    print(gold)  # all dragon heads chopped
else:
