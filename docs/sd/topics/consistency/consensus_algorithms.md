...
![](assets/Pasted%20image%2020251230123907.png)
...
5. New leader send *COORDINATOR* message to all nodes
...
- $O(n^2)$ message complexity in worst case
...
- Guaranteed Termination, deterministic
- $O(n)$ message complexity
- Works with unreliable communication, simple to implement
...
$$
\begin{align*}
N &= R + W, \quad R = \text{Read Replicas}, W = \text{Write Replicas} \\
\text{Quorum: } Q &= \frac{N}{2} + 1, \quad N = \text{Total Replicas}
\end{align*}
...
$$
\begin{align*}
& R + W > N: \quad \text{Strong consistency} \\
& R + W \leq N: \quad \text{Eventual consistency} \\
& W > \frac{N}{2}: \quad \text{Consistency for writes} \\
& R > \frac{N}{2}: \quad \text{Consistency for reads}
\end{align*}
...
4. Lock considered acquired if:
    1. Acquired on majority of instances (N/2+1)
    2. Total time < lock validity time
