...
* You can define your own protocol and make your client send data in it and server parses & process it
...
* Because there is no need to set up a new TCP connection every time, we get really low latency in communication.
...
* Above method does not work correctly in case of Scaling Storage.
...
* You can seamlessly create the file, replace the file, delete the file, read the entire file or segments of it. But these are not full-fledged file systems.
* Advantages 
...
* Bloom filters are *approximate & probabilistic data structures* that say with 100% confidence that an element doesn’t belong to a set.
* For example: Instagram wants to recommend reels but it doesn’t want to recommend something you already saw.
* Naive Way : keep track of everything that a user saw in a set, which will cause space-constraints
* To check existence of a key, we have to go through all the elements. A Set will be a BST with logarithmic insertion and lookup.
* Key Insight: Once something is inserted, you can’t take it out. When storing the actual data is not worth it, we use bloom filters.
...
* Advantage:
    * Space Efficiency
    * False Positive Rate (increases as more and more keys are inserted)
    * Hence, when the number of keys increases, we have to recreate the bloom filter with a larger size & populate keys again. So estimate max keys and provision a large one to start with.
...
* Practical Application
    ...
    * You need a definitive No with 100% certainty
...
* Moving the data could be even more expensive than the request
